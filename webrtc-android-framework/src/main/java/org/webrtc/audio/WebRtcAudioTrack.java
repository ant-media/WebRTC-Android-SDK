...
public class WebRtcAudioTrack {
  private static final String TAG = "WebRtcAudioTrackExt";
  ...
  public WebRtcAudioTrack(Context context, AudioManager audioManager) {
    this(context, audioManager, null /* errorCallback */, contentType);
  }

  public WebRtcAudioTrack(
      Context context, AudioManager audioManager, @Nullable AudioTrackErrorCallback errorCallback, int contentType) {
    threadChecker.detachThread();
    this.context = context;
    this.audioManager = audioManager;
    this.errorCallback = errorCallback;
    this.volumeLogger = new VolumeLogger(audioManager);
    WebRtcAudioTrack.contentType = contentType;
  }
  ...
  private static AudioTrack createAudioTrackOnLollipopOrHigher(
    ...
    int contentTypeLocal = AudioAttributes.CONTENT_TYPE_SPEECH;
    if (contentType != -1) {
      contentTypeLocal = contentType;
    }
    // Create an audio track where the audio usage is for VoIP and the content type is speech.
    return new AudioTrack(new AudioAttributes.Builder()
                              .setUsage(DEFAULT_USAGE)  //AudioAttributes.USAGE_MEDIA) //DEFAULT_USAGE)
                              .setContentType(contentTypeLocal)
                              .build(),
  ...
  private static AudioTrack createAudioTrackOnLowerThanLollipop(
      int sampleRateInHz, int channelConfig, int bufferSizeInBytes) {
    int contentTypeLocal = AudioManager.STREAM_VOICE_CALL;
    if (contentType != -1) {
      contentTypeLocal = contentType;
    }
    return new AudioTrack(contentTypeLocal, sampleRateInHz, channelConfig,

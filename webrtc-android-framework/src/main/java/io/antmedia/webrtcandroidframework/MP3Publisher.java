...
public class MP3Publisher {
    ...
    public void startFilePublishing() {
        try {
                /*
            ...
            MediaFormat format = getMediaFormat(extractor);
            ...
            MediaCodec codec = getMediaCodec(format);

            readAndPublishFile(extractor, format, codec);
    ...
    @NonNull
    public MediaCodec getMediaCodec(MediaFormat format) throws IOException {
        // Create a MediaCodec to decode the MP3 file
        MediaCodec codec = getCodecByName(format);
        codec.configure(format, null, null, 0);
        codec.start();
        return codec;
    }

    public MediaCodec getCodecByName(MediaFormat format) throws IOException {
        return MediaCodec.createDecoderByType(format.getString(MediaFormat.KEY_MIME));
    }
    ...
    public void readAndPublishFile(MediaExtractor extractor, MediaFormat format, MediaCodec codec) throws InterruptedException {
        CustomWebRtcAudioRecord audioInput = webRTCClient.getAudioInput();

        int bufferLength = audioInput.getBufferByteLength(); // this is the length of 10ms data
        ByteBuffer rawAudioBuffer = getRawAudioByteBuffer(bufferLength);
    ...
    @NonNull
    public ByteBuffer getRawAudioByteBuffer(int bufferLength) {
        ByteBuffer rawAudioBuffer = ByteBuffer.allocate(bufferLength *20);
        return rawAudioBuffer;
    }

    @Nullable
    private MediaFormat getMediaFormat(MediaExtractor extractor) {

...
public class WebRTCClient implements IWebRTCClient ,AppRTCClient.SignalingEvents, PeerConnectionClient.PeerConnectionEvents
{
  ...
  public void startStream(String url, String streamId, String mode){
    ...
    for (String permission : CallActivity.MANDATORY_PERMISSIONS) {
      if (this.activity.checkCallingOrSelfPermission(permission) != PackageManager.PERMISSION_GRANTED) {
        Toast.makeText(this.activity, "Permission " + permission + " is not granted", Toast.LENGTH_SHORT).show();
        return;
      }
    }
